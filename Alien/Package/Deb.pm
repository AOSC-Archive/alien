...
sub prep {
	...
	$this->do("mkdir -p $dir/autobuild/override/DEBIAN") ||
		die "mkdir $dir/autobuild/override/DEBIAN failed: $!";
...
#	close OUT;
	...
	# Control file.
	open (OUT, ">$dir/autobuild/defines") || die "$dir/autobuild/defines: $!";
	print OUT "PKGNAME=".$this->name."\n";
	print OUT "PKGSEC=alien\n";
#	print OUT "Priority: extra\n";
	print OUT "MTER=\"".$this->username." <".$this->email.">\"\n";
	print OUT "\n";
...
#	}
	print OUT "PKGDEP=lsb\n";
	print OUT "PKGDES=\"".$this->summary."\"\n";
	print OUT "PKGVER=".$this->version."\n";
#	print OUT "PKGDEP=Aliened"
...
sub build {
	...
	my $log=$this->runpipe(1, "autobuild 2>&1");
#	chdir "..";
	my $err=$?;
	if ($err) {
		if (! defined $log) {
			die "Package build failed; could not run autobuild.\n";
		}
		die "Package build failed. Here's the log:\n", $log;
	}

	$this->do("mv abdist.deb ../".$this->name."_".$this->version."-".$this->release."_".$this->arch.".deb") ||
		die("No abdist.deb ");
	chdir "..";
...
sub cleantree {
	...
	$this->do("rm", "-rf", "$dir/autobuild $dir/abdist");
}
...
sub savescript {
	...
	next if $data =~ m/^\s*$/;
	open (OUT,">$dir/autobuild/override/DEBIAN/$script") ||
